...
function ssLog(step, msg) {
  const ts = new Date().toISOString().slice(11, 23);
  console.log(`[screenshot][${ts}] ${step}: ${msg}`);
}

async function launchBrowser() {
  ssLog('launch', 'Starting browser launch...');
  try {
    ssLog('launch', 'Importing @sparticuz/chromium...');
    const chromium = (await import('@sparticuz/chromium')).default;
    ssLog('launch', 'Chromium module loaded');

    ssLog('launch', 'Resolving executablePath...');
    const execPath = await chromium.executablePath();
    ssLog('launch', 'executablePath = ' + execPath);

    const fsp_check = require('fs/promises');
    try {
      await fsp_check.access(execPath);
      ssLog('launch', 'Binary exists and is accessible');
    } catch (e) {
      ssLog('launch', 'ERROR: Binary not accessible: ' + e.message);
      throw new Error('Chromium binary not found at ' + execPath);
    }

    ssLog('launch', 'Importing puppeteer-core...');
    const puppeteer = (await import('puppeteer-core')).default;
    ssLog('launch', 'puppeteer-core loaded');
    ...
    ssLog('launch', 'Launch args: ' + args.join(' '));
    ...
    ssLog('launch', 'Browser launched successfully. PID: ' + browser.process()?.pid);
    browser.on('disconnected', () => {
      ssLog('lifecycle', 'Browser disconnected');
      _browser = null;
      _launchPromise = null;
    });
    ...
    ssLog('launch', 'FAILED: ' + err.message);
...
async function getBrowser() {
  ...
  _launchError = null;
  _launchPromise = launchBrowser().then(b => { _browser = b; return b; }).catch(e => {
    _launchPromise = null;
    throw e;
  });
...
async function screenshotWidget(embedUrl) {
  const t0 = Date.now();
  const log = (step, msg) => ssLog(step, `[${Date.now() - t0}ms] ${msg}`);

  log('start', 'URL: ' + embedUrl.substring(0, 120));

  log('auth', 'Getting access token...');
  const token = await getAccessToken();
  log('auth', 'Token acquired');

  log('browser', 'Getting browser...');
  ...
  try {
    ...
    log('page', 'Auth header set');

    // Listen for console messages from the page (useful for debugging)
    page.on('console', msg => log('page-console', msg.type() + ': ' + msg.text().substring(0, 200)));
    page.on('pageerror', err => log('page-error', err.message.substring(0, 200)));
    page.on('requestfailed', req => log('req-fail', req.url().substring(0, 100) + ' ' + (req.failure()?.errorText || '')));

    log('nav', 'Navigating...');
    const response = await page.goto(embedUrl, { waitUntil: 'networkidle2', timeout: 25000 });
    log('nav', 'Status: ' + response.status() + ', final URL: ' + page.url().substring(0, 100));

    // Detect auth redirect
    const finalUrl = page.url();
    if (finalUrl.includes('/login') || finalUrl.includes('/oidc') || finalUrl.includes('accounts.')) {
      log('nav', 'AUTH REDIRECT detected — embed requires different auth');
      throw new Error('Redirected to login: ' + finalUrl.substring(0, 100));
    }

    // Wait for chart content — try multiple selectors
    const selectors = [
      'canvas',
      'svg.marks',
      'svg[class*="vega"]',
    ...
    log('render', 'Waiting for chart selectors...');
    let foundSelector = null;
    for (const sel of selectors) {
      try {
        await page.waitForSelector(sel, { timeout: 2500 });
        foundSelector = sel;
        log('render', 'FOUND: ' + sel);
        break;
      } catch (_) {}
    }
    if (!foundSelector) {
      log('render', 'WARN: No chart selector found. Using 4s fallback wait.');
      await new Promise(r => setTimeout(r, 4000));
    } else {
      // Extra settle time for animations/transitions
      log('render', 'Waiting 2s for animations to settle...');
      await new Promise(r => setTimeout(r, 2000));
    }

    // Get page dimensions for smart clipping
    const bodyBox = await page.evaluate(() => ({
      w: document.body.scrollWidth,
      h: document.body.scrollHeight,
    }));
    log('render', 'Page dimensions: ' + bodyBox.w + 'x' + bodyBox.h);

    log('capture', 'Taking screenshot...');
    const png = await page.screenshot({
      type: 'png',
      clip: { x: 0, y: 0, width: 900, height: Math.min(bodyBox.h, 600) },
    });
    log('capture', 'Done. Size: ' + png.length + ' bytes (' + (Date.now() - t0) + 'ms total)');
...
async function handleApi(req, res, url) {
  ...
  if (req.method === 'GET' && url.pathname === '/api/proxy/screenshot-health') {
    const diag = { ok: false, browserConnected: !!(_browser?.connected), lastError: _launchError };
    try {
      const browser = await getBrowser();
      const page = await browser.newPage();
      ...
      await page.close();
      diag.ok = true;
      diag.chromiumVersion = await browser.version();
      sendJson(res, 200, diag);
    } catch (err) {
      diag.error = err.message;
      sendJson(res, 500, diag);
    }
    return true;
  }
  ...
  if (req.method === 'GET' && url.pathname === '/api/proxy/widget-screenshot') {
    ...
    try {
      ...
      ssLog('endpoint', 'Error: ' + err.message);
      sendJson(res, 500, { error: 'Screenshot failed', detail: err.message });
      return true;
    }
  }
